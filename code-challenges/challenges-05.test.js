...
const sortStarWarsCharacters = (starWarsArr) => {
  // loop through starwarsarray
  // create a reference to the last index in the starwarsarray
  // build a while loop within the for loop that runs while the current index + 1 is less than the index just created
  // create an if statement that checks if the current variable height in the iteration is less than the variable height at the variable at index + 1
  // declare a temp variable with the height of (starWars[i])
  // declare starwars[i] then assign to the value of starwars at i+1
  // then assign starwars[i+1] as the temp var
  // decrement the last index by 1
  // return starwarsarr
  for (let i = 0; i < starWarsArr.length; i++) {
    let lastObjectIndex = starWarsArr.length - 1;
    while (i + 1 < lastObjectIndex) {
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(character => {
    let array = character.match(/^[0-9]+\b\s\w+\S+\s/);
    let array2 = character.replace(array,'');
    result.push(array2);
  });
  return result;
};
